...
exports.sendEmailCampaign = catchAsync(async (req, res, next) => {
  ...
  try {
    ...
    const message = `New Wallet Transaction of Rs.${(
      campaignDoc.amount * 1
    ).toFixed(2)} Proccessed successfully towards Mail Campaign broadcast.`;
  ...
  const emailHTML = campaignDoc.html
    .replace(/&lt;/g, "<")
    .replace(/&gt;/g, ">");
...
exports.sendSMSCampaign = catchAsync(async (req, res, next) => {
  ...
  try {
    ...
    const message = `New Wallet Transaction of Rs.${(
      campaignDoc.amount * 1
    ).toFixed(2)} Proccessed successfully towards SMS Campaign broadcast.`;
    const options = notification_options;
